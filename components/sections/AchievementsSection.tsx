      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        >
          {stats.map((stat, index) => (
            <div
              key={stat.label}
              className="bg-foreground/5 backdrop-blur-md border border-foreground/10 rounded-xl p-4 md:p-6 text-center hover:border-[#DFFF00]/30 transition-colors"
            >
              <div className="w-8 h-8 bg-[#DFFF00]/10 rounded-lg flex items-center justify-center text-racing-red mx-auto mb-2">
                {stat.icon}
              </div>
              <div className="font-lando text-2xl md:text-4xl text-foreground mb-1">{stat.value}</div>
              <div className="font-tech text-[10px] md:text-xs uppercase tracking-wider text-foreground/50">{stat.label}</div>
            </div>
          ))}
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {achievements.map((category, catIndex) => (
            <motion.div
              ...
              className="bg-foreground/5 backdrop-blur-md border border-foreground/10 rounded-2xl p-6"
            >
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 bg-[#DFFF00]/10 rounded-lg flex items-center justify-center text-racing-red">
                  {category.icon}
                </div>
              ...
              <div className="space-y-4">
                {category.items.map((item, itemIndex) => (
                  <div key={itemIndex} className="border-l-2 border-foreground/10 pl-4 hover:border-[#DFFF00] transition-colors">
                    <div className="flex items-baseline gap-2 mb-1">
                      <span className="font-lando text-2xl text-racing-red">{item.metric}</span>
                      <span className="font-tech text-sm text-foreground">{item.label}</span>
                    </div>
                    <p className="font-tech text-xs text-foreground/50">{item.desc}</p>
                  </div>
                ))}
              </div>
            </motion.div>
